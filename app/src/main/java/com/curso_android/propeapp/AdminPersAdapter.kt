...
import com.curso_android.propeapp.databinding.ListItemPersonalBinding
...
class AdminPersAdapter(private var personal: List<AdminPers> = emptyList(), private val alSeleccionarPersonal:(String) -> Unit) : RecyclerView.Adapter<AdminPersAdapter.AdminPersViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AdminPersViewHolder {
        val binding = ListItemPersonalBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class AdminPersViewHolder(private val binding: ListItemPersonalBinding) : RecyclerView.ViewHolder(binding.root) {
